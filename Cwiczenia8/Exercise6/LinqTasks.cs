...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            var methodSyntax = Emps.Where(e => e.Salary == Emps.Max(x => x.Salary));
        ...
        {
            var methodSyntax = Emps.GroupBy(e => e.Job).Select(emp => new
            {
                Praca = emp.Key,
        ...
        {
            var methodSyntax = Emps.Count(e => e.Job == "Backend programmer") > 0;
        ...
        public static Emp Task9()
        {
            var methodSyntax = Emps.Where(e => e.Job == "Frontend programmer" )
                .OrderByDescending(x => x.HireDate).ElementAt(0);
        ...
        {
            var element = new List<dynamic>();
            element.Add(new { Ename = "Brak wartości", Job = (string?)null, HireDate = (DateTime?)null });
        ...
        public static IEnumerable<object> Task11()
        {
            var methodSyntax = Emps.Join(
                ...
                emp => emp.Deptno,
                dept => dept.Deptno,
                (emp, dept) => new { dept.Dname }).GroupBy(e => e.Dname).Select(x => new
            {
                name = x.Key,
                numOfEmployees = x.Count()
            }).Where(xd => xd.numOfEmployees > 0);
        ...
        {
            IEnumerable<Emp> result = Emps.Get();
            return result;
        }
        ...
        {
            var methodSyntax = arr.GroupBy(e => e)
                .Select(o => new
                {
                    Number = o.Key,
                    HowMany = o.Count()
                }).Where(element => element.HowMany % 2 != 0).Select(o => o.Number)
                .ElementAt(0);
        ...
        {
            var methodSyntax = Depts.GroupJoin(
                    Emps,
                    dept => dept.Deptno,
                    emp => emp.Deptno,
                    (dept, empGroup) => new
    ...
    public static class CustomExtensionMethods
    {
        ...
        /// Metoda powinna zwrócić tylko tych pracowników, którzy mają min. 1 bezpośredniego podwładnego.
        /// Pracownicy powinny w ramach kolekcji być posortowani po nazwisku (rosnąco) i pensji (malejąco).
        public static IEnumerable<Emp> Get(this IEnumerable<Emp> employees)
        {
            return employees.Where(emp => employees.Any(x => x.Mgr != null && x.Mgr.Empno == emp.Empno))
